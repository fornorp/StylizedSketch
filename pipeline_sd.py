...
class ADPipeline(StableDiffusionPipeline):
    ...
    def optimize(
        ...
        if content_image is not None:
            ...
            # latents = torch.cat([style_latent.clone()] * batch_size)
        ...
        for i, t in enumerate(pbar):
            ...
            # print(style_latent.mean().item(), style_latent.var().item(), content_latent.mean().item(), content_latent.var().item())
            for j in range(iters):
                ...
                # s2 = ad_loss(qs_list, k_list, v_list, s_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                # style_loss = s1 + s2 * 0.2
                ...
                # content_loss = (c1 + c2) * 0.5
                # style_ref = ad_loss(q_list, kc_list, vc_list, self_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                
                # style_loss = ad_loss(qs_list, k_list, v_list, s_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                # style_loss = ad_loss(q_list, k_list, v_list, s_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                if content_image is not None:
                    content_loss = q_loss(q_list, qc_list)
                    # content_ref = q_loss(q_list, qs_list)
                    
                    # content_loss = q_loss(latents, content_latent)
                ...
                # loss = style_loss / (style_loss + style_ref + 0.01) + content_loss / (content_loss + content_ref + 0.01)
                # loss = style_loss + content_loss * 1.5
                
                self.accelerator.backward(loss)
                # print(latents.grad.mean().item(), latents.grad.var().item(), latents.grad.min().item(), latents.grad.max().item())
                optimizer.step()
                pbar.set_postfix(loss=loss.item(), style_loss=style_loss.item(), content_loss=content_loss.item(), time=t.item(), iter=j)
        images = self.latent2image(latents)
        # Offload all models
        self.maybe_free_model_hooks()
        return images
