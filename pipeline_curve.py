...
class ADPipeline(StableDiffusionPipeline):
    ...
    def optimize(
        ...
        null_embeds_for_vec = null_embeds.repeat(latents.shape[0], 1, 1)
        ...
        if content_image is not None:
            ...
            ## canny
            canny_latent = self.image2latent(canny_image)
            dtype = next(self.vae.parameters()).dtype
            device = self._execution_device
            canny_image = canny_image.to(device=device, dtype=dtype)
        ...
        self.scheduler.set_timesteps(num_inference_steps)
        timesteps = self.scheduler.timesteps
        #### optimize points (clipasso)
        # optimizer_p = self.accelerator.prepare(optimizer.points_optim)
        # init = vec_image.get_image()

        #### optimize mlp (clipascene)
        with torch.no_grad():
        ...
        # init svg training
        renderer.train_color = False
        v_iter = 200
        for j in range(v_iter):
            inputs, points_init = dataset.__getitem__(0)
            cur_img, points_opt, shapes, shape_groups = renderer.get_image(points_init)
            cur_img = cur_img.to(device)
            optimizer_p.zero_grad()
            loss_vec = clip_loss(cur_img, content_image.to(device))            
            self.accelerator.backward(loss_vec)
            optimizer_p.step()
        with torch.no_grad():
            inputs, points_init = dataset.__getitem__(0)
            cur_img, points_opt, shapes, shape_groups = renderer.get_image(points_init)
            init_svg = cur_img.clone()
            cur_img = cur_img.to(device)
            vec_latent = self.image2latent(cur_img).float()
        
        
        #### optimize latent
        ## init with vec form
        # latents = vec_latent.clone()
        # latents = latents.detach().float()
        ## init with canny form
        # latents = canny_latent.clone()
        # latents = latents.detach().float()
        
        
        optimizer_b = torch.optim.Adam([latents.requires_grad_()], lr=lr)
        optimizer_b = self.accelerator.prepare(optimizer_b)

        pbar = tqdm(timesteps, desc="Optimize")
        steps = 0
        renderer.train_color = True
        ...
        for i, t in enumerate(pbar):
            ...
            v_iter = 3
            recon = self.latent2image(latents)
            for j in range(v_iter):
                # steps += 1
                inputs, points_init = dataset.__getitem__(0)
                cur_img, points_opt, shapes, shape_groups = renderer.get_image(points_init)
                ...
                optimizer_p.zero_grad()
                # match_loss_vec = loss_fxn(vec_latent, latents.detach())
                perceptual_loss_1 = clip_loss(cur_img, content_image.to(device))
                perceptual_loss_2 = clip_loss(cur_img, recon.detach())
                # alpha = steps * 0.005
                alpha = 0.2 + steps * 0.003
                if steps == 199:
                    alpha = 1.0
                perceptual_loss = (1 - alpha) * perceptual_loss_1 + alpha * perceptual_loss_2
                # perceptual_loss = perceptual_loss_1 + perceptual_loss_2
                loss_vec = perceptual_loss # + match_loss_vec
            ...
            with torch.no_grad():
                ...
                qv_list, kv_list, vv_list, v_out_list = self.extract_feature(
                    vec_latent,
                    # canny_latent,
                    t,
                    null_embeds_for_vec,
                ...
                '''
                qe_list, ke_list, ve_list, e_out_list = self.extract_feature(
                    #vec_latent,
                    canny_latent,
                    t,
                    null_embeds_for_vec,
            ...
            for j in range(iters):
                ...
                s1 = ad_loss(q_list, ks_list, vs_list, self_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                style_loss = s1
                c1 = ad_loss(qc_list, k_list, v_list, self_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                c2 = ad_loss(q_list, kc_list, vc_list, c_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                ## fxn: change the weight (default: 0.75)
                content_loss = (c1 + c2) * 0.75
                ## orig
                ...
                beta = 1.0 - steps * 0.005
                gamma = 0.5 - steps * 0.005
                if gamma < 0:
                    gamma = 0
                
                # beta = 0.4 - steps * 0.002
                ## fxn: change the schedule of beta
                match_loss_bit = q_loss(q_list, qv_list) + (q_loss(k_list, kv_list) + q_loss(v_list, vv_list)) * beta
                # match_loss_bit = qmin_loss(q_list, qv_list, qe_list) 
                # match_loss_bit += (qmin_loss(k_list, kv_list, ke_list) + qmin_loss(v_list, vv_list, ve_list)) * beta
                # match_loss_bit = qmin_loss(q_list, qv_list, qe_list)
                # sv = ad_loss(q_list, kv_list, vv_list, self_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                # se = ad_loss(q_list, ke_list, ve_list, self_out_list, scale=attn_scale, source_mask=source_mask, target_mask=target_mask)
                # match_loss_bit += (sv + se) * beta
                # list_vs = ad_loss_list(qv_list, ks_list, vs_list, v_out_list)
                # list_es = ad_loss_list(qe_list, ks_list, vs_list, e_out_list)
                # print(list_vs, list_es)
                # match_loss_bit += (qmin_loss(k_list, kv_list, ke_list) + qmin_loss(v_list, vv_list, ve_list)) * beta
                # match_loss_bit = weighted_q_loss(q_list, qv_list, v_out_list, qe_list, e_out_list, ks_list, vs_list)
                
                # match_loss_bit = (c3 + c4) * 0.5
                # alpha = steps * 0.005 - 0.003
                # loss_bit = match_loss_bit * weight * 3 + style_loss # + direct_loss_bit
                # loss = style_loss + weight * content_loss
                # loss = style_loss * 0.01 + perceptual_loss

                ## edge sparsity
                # recon = self.latent2image(latents)
                # recon = 1.0 - recon
                # sparse_loss = (recon * canny_image).mean()
                # color_loss = torch.abs(recon - recon_gray).mean()
                
                loss_bit = content_loss + style_loss + match_loss_bit * weight # + sparse_loss
                self.accelerator.backward(loss_bit)
                optimizer_b.step()
                pbar.set_postfix(style=style_loss.item(), 
                                 content=content_loss.item(), 
                                 match=match_loss_bit.item(), 
                                 # sparse=sparse_loss.item(), 
                                 time=t.item(), iter=j)
            
        #### latent
        images = self.latent2image(latents)
        canny = self.latent2image(canny_latent)
        #### clipasso
        # cur_img = vec_image.get_image()
        #### clipascene
        with torch.no_grad():
            inputs, points_init = dataset.__getitem__(0)
            result, points_opt, shapes, shape_groups = renderer.get_image(points_init)
            renderer.save_svg(f"tests", f"final", 0, shapes, shape_groups)
            
        # Offload all models
        self.maybe_free_model_hooks()
        return images, result, init_svg
        # return image
